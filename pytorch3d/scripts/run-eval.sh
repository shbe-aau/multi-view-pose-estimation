...
if test "$DATA_SPLIT" = "tless"; then
    SUB_DATASET="-primsesense"
else
    SUB_DATASET=""
fi
...
if test -d "$LOCAL_BOP_EVAL_PATH"; then
    ...
    #fi
    ${MERGED_DOCKER} python bop_toolkit/scripts/eval_bop19.py --result_filenames=${RESULT_CSV} --targets_filename=${TARGETS_FILE} --datasets_path /shared-folder/bop/bop-${DATASET}-dataset/ --eval_path ${BOP_EVAL_PATH}
fi

echo "Show BOP performance"
${MERGED_DOCKER} python bop_toolkit/scripts/show_performance_bop19.py --result_filenames=${RESULT_CSV} --datasets_path /shared-folder/bop/bop-${DATASET}-dataset/ --eval_path ${BOP_EVAL_PATH}
