...
BOP_PICKLE_PATH=${SHARED_FOLDER}/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/tless-${DATA_SPLIT}-obj${OBJ_ID}.p
if test -f "$BOP_PICKLE_PATH"; then
    echo " - BOP dataset in pickle format already exists, skipping..."
else
    if test "$DATA_SPLIT" = "test"; then
	${MERGED_DOCKER} python bop_toolkit/scripts/bop2pickle.py --datasets_path /shared-folder/bop/bop-tless-dataset/ --scene_ids ${SCENE_IDS} --obj_ids ${OBJ_ID} --dataset_split ${DATA_SPLIT} #> log.out
    else
...
if test "$APPROACH_NAME" = "sundermeyer"; then
    SM_PICKLE_PATH=${SHARED_FOLDER}/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/${APPROACH_NAME}-result-${DATA_SPLIT}-obj${OBJ_ID}.p
    if test -f "$SM_PICKLE_PATH"; then
	echo " - pickle with results from Sundermeyers approach already exists, skipping..."
    else
	cp ${SHARED_FOLDER}/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/tless-${DATA_SPLIT}-obj${OBJ_ID}.p ${SM_PICKLE_PATH}
	wait
	${AAE_DOCKER} python -m auto_pose.ae.images2poses cad_autoencoder/obj${OBJ_ID} /shared-folder/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/sundermeyer-result-${DATA_SPLIT}-obj${OBJ_ID}.p > log.out
	wait
	echo "Convert results from Sundermeyer to CSV for BOP toolkit"
	SM_CSV_PATH=${SHARED_FOLDER}/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/${APPROACH_NAME}-obj${OBJ_ID}_tless-${DATA_SPLIT}-primesense.csv
	if test -f "$SM_CSV_PATH"; then
	    echo " - CSV with results from Sundermeyers approach already exists, skipping..."
	else
	    ${MERGED_DOCKER} python pytorch3d/eval-pickle.py -pi /shared-folder/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/${APPROACH_NAME}-result-${DATA_SPLIT}-obj${OBJ_ID}.p -o /shared-folder/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/sundermeyer-obj${OBJ_ID}_tless-${DATA_SPLIT}-primesense.csv > log.out #-op pytorch3d/data/t-less-obj${OBJ_ID}/cad/obj_${OBJ_ID}.ply
	    wait
	fi
    fi
else
    OUR_CSV_PATH=${SHARED_FOLDER}/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/${APPROACH_NAME}-obj${OBJ_ID}_tless-${DATA_SPLIT}-primesense.csv
	...
	${MERGED_DOCKER} python pytorch3d/eval-pickle.py -pi /shared-folder/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/tless-${DATA_SPLIT}-obj${OBJ_ID}.p -o /shared-folder/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/${APPROACH_NAME}-obj${OBJ_ID}_tless-${DATA_SPLIT}-primesense.csv -ep ${ENCODER_WEIGHTS} -mp ${TRAINED_MODEL} > log.out #-op pytorch3d/data/t-less-obj${OBJ_ID}/cad/obj_${OBJ_ID}.ply
	wait
    fi
...
echo "Run BOP eval"
RESULT_CSV=/shared-folder/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/${APPROACH_NAME}-obj${OBJ_ID}_tless-${DATA_SPLIT}-primesense.csv
BOP_EVAL_PATH=/shared-folder/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/eval
LOCAL_BOP_EVAL_PATH=${SHARED_FOLDER}/bop/bop-tless-dataset/tless/pickles/obj${OBJ_ID}/eval/${APPROACH_NAME}-obj${OBJ_ID}_tless-${DATA_SPLIT}-primesense
if test -d "$LOCAL_BOP_EVAL_PATH"; then
    echo " - eval dir from BOP already exists, skipping..."
else
    ${MERGED_DOCKER} python bop_toolkit/scripts/eval_bop19.py --result_filenames=${RESULT_CSV} --targets_filename=/shared-folder/bop/bop-tless-dataset/tless/targets/${DATA_SPLIT}_targets_obj${OBJ_ID}.json --datasets_path /shared-folder/bop/bop-tless-dataset/ --eval_path ${BOP_EVAL_PATH}
fi
